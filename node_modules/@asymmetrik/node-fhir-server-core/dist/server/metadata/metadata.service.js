...
let generateCapabilityStatement = ({
  ...
  logger.info('Metadata.generateCapabilityStatement'); // create profile list
  ...
  let deprecatedLogger = deprecate(container.get('default'), 'Using the logger this way is deprecated. Please see the documentation on ' + 'BREAKING CHANGES in version 2.0.0 for instructions on how to upgrade.'); // Get the necessary functions to generate statements

  let {
    makeStatement,
    securityStatement
  } = statementGenerator(fhirVersion, deprecatedLogger); // If we do not have these functions, we cannot generate a new statement
  ...
  serverStatement.resource = active_profiles.map(profile => {
    ...
    let resource = customMakeResource ? customMakeResource(Object.assign(fhirVersion, {
      key: profile.key
    }), deprecatedLogger) : profile.makeResource(fhirVersion, profile.key); // Determine the interactions we need to list for this profile
