...
let server; // Helper function to replace express params with mock values

let fillRoute = (route, key) => route.replace(':base_version', VERSIONS['3_0_1']).replace(':id', 1).replace(':resource', key); //Helper function build a custom security statement
...
let customMakeResource = args => {
  let Resource = resolveSchema(args.base_version, args.key); // Return our conformance statement
...
describe('Conformance Tests', () => {
  test('Test that the server returns a default capability statement', async () => {
    ...
    let config = Object.assign({}, testConfig, {
      logging: {
        level: 'emerg'
      }
    });
  ...
  test('Test that every profile gets a default resource entry ', async () => {
    ...
    let response = await request(server.app)['get']('/3_0_1/metadata');
    expect(response.body.resourceType).toBe('CapabilityStatement'); //Check that the reference for each resource is the default
  ...
  test('Test that every profile gets a custom resource entry ', async () => {
    ...
    mock_service.makeResource = customMakeResource; // Standup a basic server
    ...
    let {
      routes
    } = require('../route.config'); //Patch the test config to include our custom makeResource
    ...
    let response = await request(server.app)['get']('/3_0_1/metadata');
    expect(response.body.resourceType).toBe('CapabilityStatement'); //Check the reference for each resource is the customised one
  ...
  test('Test that every profile gets a custom resource entry from config', async () => {
    ...
    server = new Server(config).setProfileRoutes().setErrorRoutes();
    let keys = Object.keys(server.config.profiles); //Patch the test config to include our custom metadata module
