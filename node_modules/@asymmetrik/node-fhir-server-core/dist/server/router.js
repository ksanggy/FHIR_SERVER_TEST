...
const {
  getController
} = require('./utils/controllers.utils');
...
function enableBaseRoute(app, config, corsDefaults) {
  ...
  for (let i; routes.length; i++) {
    ...
    app.options(routes[i].path, cors(corsOptions)); // Enable base route

    app[routes[i].type](routes[i].path, cors(corsOptions), versionValidationMiddleware(versionValidationConfiguration), sanitizeMiddleware(routes[i].args), routes[i].controller({
      config
    }));
  }
}
...
function setRoutes(options = {}) {
  ...
  enableResourceRoutes(app, config, corsDefaults); // Enable all routes, operations base: Batch and Transactions
