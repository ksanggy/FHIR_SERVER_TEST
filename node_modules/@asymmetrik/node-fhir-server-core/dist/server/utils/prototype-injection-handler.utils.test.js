...
describe('Prototype Injection Handler', () => {
  ...
  test('it should return an operation outcome if a request body with "__proto__" comes in', async done => {
    const test = '{"resourceType":"Parameters","parameter":[{"name":"ndc","valueString":"00006404500","__proto__":{ "b":5}},{"name":"gcnseqno","valueString":"1417"}]}';
    request(server.app).post('/3_0_1/Patient').set('Accept', 'application/fhir+json').set('Content-Type', 'application/fhir+json').send(JSON.parse(test)).then(response => {
