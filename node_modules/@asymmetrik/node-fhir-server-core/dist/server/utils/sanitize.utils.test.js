...
const {
  sanitizeMiddleware
} = require(path.resolve('./src/server/utils/sanitize.utils'));

const {
  VERSIONS
} = require(path.resolve('./src/constants'));
...
describe('Sanitize Utils Tests', () => {
  test('should not pass an error if no args are provided and none are required', () => {
    ...
    let next = jest.fn(); // invoke our middleware

    middleware(req, null, next); // Make sure next was called but without an error
  ...
  test('should coerce the arguments into the correct type', () => {
    ...
    let params = {
      first_name: 'Joe',
      age: '24',
      birthdate: '740088404220',
      isAlive: 'true'
    };
    let req = {
      params: Object.assign(params, ARGS_PARAM)
    };
    let next = jest.fn(); // invoke our middleware

    middleware(req, null, next); // Inspect params and make sure they are the correct type
    ...
    expect(typeof birthdate).toEqual('string'); // birthdate no longer moment object
    // expect(birthdate).toBeInstanceOf(moment);
    // Make sure next was called but without an error

    expect(next).toHaveBeenCalled(); // This should be the argument next would be invoked with
  ...
  test('should filter out extra arguments that do not belong', () => {
    ...
    middleware(req, null, next); // only id should be passed through
    ...
    expect(req.sanitized_args.age).toBeUndefined(); // Make sure next was called but without an error
  ...
  test('should allow a valid identifier to be passed without stripping characters', () => {
    let middleware = sanitizeMiddleware(ARGS);
    let params = {
      identifier: 'http://www.example.com|2334567'
    };
    let req = {
      params: Object.assign(params, ARGS_PARAM)
    };
  ...
  test('should pass an error to next if something other than a string is passed in', () => {
    let middleware = sanitizeMiddleware(ARGS); // age should be a number, but express should make everything a string,
  ...
  test('should allow all common search args', () => {
    ...
    middleware(req, null, next); // console log error so easier to find
    ...
    expect(typeof _elements).toEqual('string'); // Make sure next was called but without an error
