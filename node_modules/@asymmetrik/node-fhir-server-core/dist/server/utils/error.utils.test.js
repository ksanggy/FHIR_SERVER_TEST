...
describe('Error Utils Tests', () => {
  ...
  test('should return correct OperationOutcome for an unauthorized error', () => {
    ...
    let issueWithoutMessage = errorWithoutMessage.issue[0]; // Check the code and severity on one

    expect(issueWithMessage.code).toBe(ISSUE.CODE.FORBIDDEN);
    expect(issueWithoutMessage.severity).toBe(ISSUE.SEVERITY.ERROR); // Check the message for both
  ...
  test('should return correct OperationOutcome for an internal error', () => {
    ...
    let customIssueWithMessage = responseCustom.issue[0]; // Check the code and severity on one

    expect(issueWithMessage.code).toBe(ISSUE.CODE.EXCEPTION);
    expect(issueWithoutMessage.severity).toBe(ISSUE.SEVERITY.ERROR);
    expect(customIssueWithMessage.severity).toBe(ISSUE.SEVERITY.WARNING); // Check the message for both
