...
function verifyAndLoadProfiles(profiles) {
  let errors = []; // At this stage, we are only validating the configuration, not the server's
  ...
  Object.getOwnPropertyNames(profiles).forEach(name => {
    ...
    let message; // No service provided, we cannot load it
...
function validate(config) {
  ...
  invariant(!config.server.ssl || config.server.ssl && config.server.ssl.key && config.server.ssl.cert, 'Invalid SSL Configuration, Please see the Wiki for a guide on how to setup SSL. ' + 'See https://github.com/Asymmetrik/node-fhir-server-core/blob/master/docs/ServerConfiguration.md'); // If we have no profiles configured, notify them now

  invariant(Object.keys(config.profiles).length > 0, 'No profiles configured. We do not enable any profiles by default so please ' + 'review the profile wiki for how to enable profiles and capabilities. ' + 'See https://github.com/Asymmetrik/node-fhir-server-core/blob/master/docs/ConfiguringProfiles.md'); // We need to verify that each provided key is valid and that the config
  ...
  invariant(errors.length === 0, 'Encountered the following errors attempting to load your provided profiles:' + `\n${errors.join('\n')}\n` + 'See https://github.com/Asymmetrik/node-fhir-server-core/blob/master/docs/ConfiguringProfiles.md');
}

class Server {
  constructor(config = {}, app) {
    // Merge in any defaults we want to set at the server level
    this.config = mergeDefaults(config); // Setup a logger for the application

    loggers.initialize(this.config.logging); // Validate the config has minimum required settings to run

    validate(this.config); // TODO: REMOVE: logger in future versions, emit notices for now

    this.logger = deprecate(loggers.get('default'), 'Using the logger this way is deprecated. Please see the documentation on ' + 'BREAKING CHANGES in version 2.0.0 for instructions on how to upgrade.'); // Use external express instance or setup new one
  ...
  configureMiddleware() {
    ...
    this.app.use(prototypeInjectionHandler); // Set favicon

    this.app.use(favicon(this.config.server.favicon || path.join(__dirname, '../assets/phoenix.ico'))); // return self for chaining
  ...
  setErrorRoutes() {
    let logger = loggers.get('default'); //Enable error tracking error handler if supplied in config
    ...
    this.app.use((err, req, res, next) => {
      ...
      const base = req.url.split('/')[1]; // Get an operation outcome for this instance

      const OperationOutcome = resolveSchema(isValidVersion(base) ? base : VERSIONS['4_0_0'], 'operationoutcome'); // If there is an error and it is an OperationOutcome
  ...
  listen(port = process.env.PORT, host = process.env.HOST, callback) {
    ...
    let server = this.config.server; // If we are missing a port, let's notify them

    invariant(port || server.port, 'Missing port. Please provide a port when initializing the server. See ' + 'https://github.com/Asymmetrik/node-fhir-server-core/blob/master/docs/ServerConfiguration.md'); // Update the express app to be in instance of createServer

    this.app = !this.env.USE_HTTPS ? http.createServer(this.app) : https.createServer({
      key: fs.readFileSync(server.ssl.key),
      cert: fs.readFileSync(server.ssl.cert)
    }, this.app); // Start the app - will listen on 0.0.0.0 [::] if host is falsy
