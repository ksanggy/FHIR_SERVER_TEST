...
<head>
    <title>Code coverage report for src/server/server.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/server</a> server.js</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">70.91% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>78/110</span>
            </div>
        
            
            <div class='fl pad1y space-right2'>
                <span class="strong">38.37% </span>
                <span class="quiet">Branches</span>
                <span class='fraction'>33/86</span>
            </div>
...
<a name='L360'></a><a href='#L360'>360</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">4x</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">4x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">const { resolveSchema, isValidVersion } = require('./utils/schema.utils');
...
function verifyAndLoadProfiles(profiles) {
  ...
  Object.getOwnPropertyNames(profiles).forEach((name) =&gt; {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!service || !versions) {
<span class="cstat-no" title="statement not covered" >      message = `Invalid ${name} configuration. Missing service and/or versions.`;</span>
    } else {
      try {
        profiles[name].serviceModule =
          typeof service === 'string' ? require(path.resolve(service)) : service;
      } catch (err) {
<span class="cstat-no" title="statement not covered" >        message = `Invalid ${name} configuration. ${err.message}`;</span>
      }
    }
...
function validate(config) {
  ...
  invariant(
    !config.server.ssl || (<span class="branch-1 cbranch-no" title="branch not covered" >config.server.ssl </span>&amp;&amp; <span class="branch-2 cbranch-no" title="branch not covered" >config.server.ssl.key </span>&amp;&amp; <span class="branch-3 cbranch-no" title="branch not covered" >config.server.ssl.cert)</span>,
...
&nbsp;
class Server {
  constructor(config = <span class="branch-0 cbranch-no" title="branch not covered" >{},</span> app) {
...
&nbsp;
    ...
    this.app = app ? <span class="branch-0 cbranch-no" title="branch not covered" >app </span>: express();
    // Setup some environment variables handy for setup
    let { server = <span class="branch-0 cbranch-no" title="branch not covered" >{} </span>} = this.config;
&nbsp;
    this.env = {
      IS_PRODUCTION: !process.env.NODE_ENV || process.env.NODE_ENV === 'production',
      USE_HTTPS: server.ssl &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >server.ssl.key </span>&amp;&amp; <span class="branch-2 cbranch-no" title="branch not covered" >server.ssl.cert </span>? <span class="branch-0 cbranch-no" title="branch not covered" >server.ssl </span>: undefined,
    };
...
&nbsp;
  ...
  configureMiddleware() {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.config.errorTracking &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >this.config.errorTracking.requestHandler)</span> {
<span class="cstat-no" title="statement not covered" >      this.app.use(this.config.errorTracking.requestHandler());</span>
    }
    // Enable stack traces
...
&nbsp;
  ...
  configureSession(session) {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (session || server.sessionStore) {
<span class="cstat-no" title="statement not covered" >      this.app.use(session || server.sessionStore);</span>
    }
    // return self for chaining
    return this;
  }
...
<span class="fstat-no" title="function not covered" >  co</span>nfigureAuthorization() {
    // return self for chaining
<span class="cstat-no" title="statement not covered" >    return this;</span>
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  co</span>nfigurePassport() {
<span class="cstat-no" title="statement not covered" >    if (this.config.auth &amp;&amp; this.config.auth.strategy) {</span>
      let { strategy } = <span class="cstat-no" title="statement not covered" >require(path.resolve(this.config.auth.strategy.service));</span>
<span class="cstat-no" title="statement not covered" >      passport.use(strategy);</span>
    }
&nbsp;
    // return self for chaining
<span class="cstat-no" title="statement not covered" >    return this;</span>
...
<span class="fstat-no" title="function not covered" >  se</span>tPublicDirectory(publicDirectory = <span class="branch-0 cbranch-no" title="branch not covered" >'')</span> {
    // Public config can come from the core config as well, let's handle both cases
    let { server = <span class="branch-0 cbranch-no" title="branch not covered" >{} </span>} = <span class="cstat-no" title="statement not covered" >this.config;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (publicDirectory || server.publicDirectory) {</span>
<span class="cstat-no" title="statement not covered" >      this.app.use(express.static(publicDirectory || server.publicDirectory));</span>
    }
    // return self for chaining
<span class="cstat-no" title="statement not covered" >    return this;</span>
  }
...
&nbsp;
  ...
  setErrorRoutes() {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.config.errorTracking &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >this.config.errorTracking.errorHandler)</span> {
<span class="cstat-no" title="statement not covered" >      this.app.use(this.config.errorTracking.errorHandler());</span>
    }
&nbsp;
...
&nbsp;
      ...
      const OperationOutcome = resolveSchema(
        isValidVersion(base) ? base : <span class="branch-1 cbranch-no" title="branch not covered" >VERSIONS['4_0_0'],</span>
        'operationoutcome'
      );
&nbsp;
      // If there is an error and it is an OperationOutcome
      if (err &amp;&amp; err.resourceType === OperationOutcome.resourceType) {
        const status = err.statusCode || <span class="branch-1 cbranch-no" title="branch not covered" >500;</span>
        res.status(status).json(err);
      } else <span class="missing-if-branch" title="else path not taken" >E</span>if (err instanceof ServerError) {
        const status = err.statusCode || <span class="branch-1 cbranch-no" title="branch not covered" >500;</span>
        res.status(status).json(new OperationOutcome(err));
      } else <span class="cstat-no" title="statement not covered" >if (err) {</span>
        const error = <span class="cstat-no" title="statement not covered" >new OperationOutcome({</span>
          statusCode: 500,
          issue: [
            {
              severity: 'error',
...
<span class="cstat-no" title="statement not covered" >        res.status(error.statusCode).json(error);</span>
      } else {
...
&nbsp;
    ...
    this.app.use((req, res) =&gt; {
      // get base from URL instead of params since it might not be forwarded
      const base = req.url.split('/')[1] || <span class="branch-1 cbranch-no" title="branch not covered" >VERSIONS['4_0_0'];</span>
&nbsp;
      let OperationOutcome;
      <span class="missing-if-branch" title="else path not taken" >E</span>if (Object.keys(VERSIONS).includes(base)) {
        OperationOutcome = resolveSchema(base, 'operationoutcome');
      } else {
        // if it's a misplaced URL, just return an R4 OperationOutcome
<span class="cstat-no" title="statement not covered" >        OperationOutcome = resolveSchema('4_0_0', 'operationoutcome');</span>
      }
&nbsp;
      // Get an operation outcome for this instance
...
&nbsp;
  // Start the server
<span class="fstat-no" title="function not covered" >  li</span>sten(port = <span class="branch-0 cbranch-no" title="branch not covered" >process.env.PORT,</span> host = <span class="branch-0 cbranch-no" title="branch not covered" >process.env.HOST,</span> callback) {
...
<span class="cstat-no" title="statement not covered" >      host = process.env.HOST;</span>
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      port = process.env.PORT;</span>
<span class="cstat-no" title="statement not covered" >      host = process.env.HOST;</span>
    }
&nbsp;
    let server = <span class="cstat-no" title="statement not covered" >this.config.server;</span>
    // If we are missing a port, let's notify them
<span class="cstat-no" title="statement not covered" >    invariant(</span>
      port || server.port,
...
<span class="cstat-no" title="statement not covered" >    this.app = !this.env.USE_HTTPS</span>
...
<span class="cstat-no" title="statement not covered" >    this.app.listen(port || server.port, host || server.host, callback);</span>
  }
}
&nbsp;
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Tue Sep 29 2020 10:58:41 GMT-0400 (Eastern Daylight Time)
            </div>
        </div>
