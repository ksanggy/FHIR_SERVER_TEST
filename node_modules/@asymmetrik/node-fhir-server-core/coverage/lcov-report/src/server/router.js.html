...
<head>
    <title>Code coverage report for src/server/router.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/server</a> router.js</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">91.67% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>110/120</span>
            </div>
...
<a name='L369'></a><a href='#L369'>369</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">5x</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">const versionValidationMiddleware = require('./middleware/version-validation.middleware.js');
 ...
 */
function getAllConfiguredVersions(profiles = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
 ...
 * @return {boolean}
 */
function hasValidService(route = <span class="branch-0 cbranch-no" title="branch not covered" >{},</span> profile = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
...
function loadController(lowercaseKey, interaction, service) {
  return (req, res, next) =&gt; {
    ...
    const fhirVersion = VERSIONS[base_version] || <span class="branch-1 cbranch-no" title="branch not covered" >VERSIONS['4_0_0'];</span> // fallback to r4 for custom baseUrl
...
&nbsp;
    ...
    let interaction =
      lowercaseMethod === 'post' ? INTERACTIONS.OPERATIONS_POST : <span class="branch-1 cbranch-no" title="branch not covered" >INTERACTIONS.OPERATIONS_GET;</span>
&nbsp;
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (profile.baseUrls &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >profile.baseUrls.length </span>&amp;&amp; <span class="branch-2 cbranch-no" title="branch not covered" >profile.baseUrls.includes('/'))</span> {
      const operationsRoute = <span class="cstat-no" title="statement not covered" >'/'.concat(op.route).replace('$', '([$])');</span>
      // Enable cors with preflight
<span class="cstat-no" title="statement not covered" >      app.options(operationsRoute, cors(corsOptions));</span>
&nbsp;
      // Enable this operation route
<span class="cstat-no" title="statement not covered" >      app[route.type](</span>
...
&nbsp;
    baseUrls.forEach((baseUrl) =&gt; {
      const metadataPath = baseUrl === '/' ? '/metadata' : <span class="branch-1 cbranch-no" title="branch not covered" >`${baseUrl}/metadata`;</span>
&nbsp;
...
&nbsp;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (inferredProfiles.length) {
    // Enable cors with preflight
...
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (profile.baseUrls &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >profile.baseUrls.includes('/'))</span> {
        let profileRoute = <span class="cstat-no" title="statement not covered" >route.path</span>
          .replace(':resource', profileName)
          .replace(':base_version/', '');
&nbsp;
        // Enable cors with preflight
<span class="cstat-no" title="statement not covered" >        app.options(profileRoute, cors(corsOptions));</span>
&nbsp;
        // Enable this operation route
<span class="cstat-no" title="statement not covered" >        app[route.type](</span>
          profileRoute,
...
function enableBaseRoute(app, config, corsDefaults) {
  ...
  for (let i; routes.length; i++) {
    let versionValidationConfiguration = <span class="cstat-no" title="statement not covered" >{</span>
      versions: getAllConfiguredVersions(config.profiles),
    };
    let corsOptions = <span class="cstat-no" title="statement not covered" >Object.assign({}, corsDefaults, {</span>
      methods: [routes[i].type.toUpperCase()],
    });
&nbsp;
    // Enable cors with preflight
<span class="cstat-no" title="statement not covered" >    app.options(routes[i].path, cors(corsOptions));</span>
    // Enable base route
<span class="cstat-no" title="statement not covered" >    app[routes[i].type](</span>
      routes[i].path,
      cors(corsOptions),
      versionValidationMiddleware(versionValidationConfiguration),
...
&nbsp;
function setRoutes(options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
