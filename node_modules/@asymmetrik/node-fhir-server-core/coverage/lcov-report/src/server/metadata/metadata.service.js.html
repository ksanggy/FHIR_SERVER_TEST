...
<head>
    <title>Code coverage report for src/server/metadata/metadata.service.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../index.html">All files</a> / <a href="index.html">src/server/metadata</a> metadata.service.js</h1>
        <div class='clearfix'>
...
<a name='L127'></a><a href='#L127'>127</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">5x</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">const generateInteractions = require('./metadata.interactions.js');
...
let getStatementGenerators = (base_version) =&gt; {
  <span class="missing-if-branch" title="else path not taken" >E</span>if (base_version) {
    return require(`./capability.${base_version}`);
  } else {
<span class="cstat-no" title="statement not covered" >    return require('./capability.4_0_0');</span>
  }
};
&nbsp;
/**
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!makeStatement || !securityStatement) {
<span class="cstat-no" title="statement not covered" >      return reject(errors.internal('Unable to generate metadata for this FHIR specification.'));</span>
    }
&nbsp;
...
&nbsp;
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (security) {
...
&nbsp;
    ...
    let operations = keys.reduce((ops, profile_name) =&gt; {
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (opsInProfile &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >opsInProfile.length)</span> {
<span class="cstat-no" title="statement not covered" >        opsInProfile.forEach(<span class="fstat-no" title="function not covered" >(o</span>pInProfile) =&gt; {</span>
          const op = <span class="cstat-no" title="statement not covered" >{</span>
...
<span class="cstat-no" title="statement not covered" >          ops.push(op);</span>
        });
      }
&nbsp;
      return ops;
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (operations &amp;&amp; operations.length) {
<span class="cstat-no" title="statement not covered" >      serverStatement.operation = operations;</span>
    }
&nbsp;
